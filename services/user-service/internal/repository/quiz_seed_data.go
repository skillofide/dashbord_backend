...
var quizAnswerKeys = []quizKey{
	...
	{"sql-m1", 2, "MongoDB"},
	{"sql-m1", 3, "Controlled concurrent access, integrity and querying"},
	{"sql-m1", 4, "Structured Query Language"},
	{"sql-m1", 5, "NoSQL"},

	// sql-m2 (5 questions)
	{"sql-m2", 1, "ARRAY_STRING"},
	{"sql-m2", 2, "It is unique and not null"},
	{"sql-m2", 3, "To reference the primary key of another table"},
	{"sql-m2", 4, "DECIMAL / NUMERIC"},
	{"sql-m2", 5, "CHAR is fixed length, VARCHAR is variable length"},

	// sql-m3 (5 questions)
	{"sql-m3", 1, "INSERT"},
	{"sql-m3", 2, "DDL"},
	{"sql-m3", 3, "DROP"},
	{"sql-m3", 4, "DELETE can use WHERE and is logged per row; TRUNCATE removes all rows quickly"},
	{"sql-m3", 5, "LIMIT"},

	// sql-m4 (5 questions)
	{"sql-m4", 1, "LIKE"},
	{"sql-m4", 2, "%"},
	{"sql-m4", 3, "Exactly one character"},
	{"sql-m4", 4, "IS NULL"},
	{"sql-m4", 5, "BETWEEN"},
	...
	// sql-m6 (5 questions)
	{"sql-m6", 1, "HAVING"},
	{"sql-m6", 2, "WHERE filters rows before grouping; HAVING filters groups after"},
	{"sql-m6", 3, "COUNT(column)"},
	{"sql-m6", 4, "All rows including those with nulls"},
	{"sql-m6", 5, "GROUP BY"},
	...
	{"sql-m7", 2, "INNER JOIN"},
	{"sql-m7", 3, "LEFT JOIN"},
	{"sql-m7", 4, "The Cartesian product of both tables"},
	{"sql-m7", 5, "LEFT JOIN B and filter WHERE B.id IS NULL"},

	// sql-m8 (5 questions)
	{"sql-m8", 1, "A query nested inside another query"},
	{"sql-m8", 2, "IN"},
	{"sql-m8", 3, "One that references a column from the outer query"},
	{"sql-m8", 4, "EXISTS"},
	{"sql-m8", 5, "In the GROUP BY clause"},

	// sql-m9 (5 questions)
	{"sql-m9", 1, "A virtual table based on a query"},
	{"sql-m9", 2, "Faster reads on the indexed column"},
	{"sql-m9", 3, "Slower writes and extra storage"},
	{"sql-m9", 4, "EXPLAIN"},
	{"sql-m9", 5, "No, not usually"},

	// sql-m10 (5 questions)
	{"sql-m10", 1, "Atomicity"},
	{"sql-m10", 2, "Consistency"},
	{"sql-m10", 3, "COMMIT"},
	{"sql-m10", 4, "Rolling back to a point mid-transaction"},
	{"sql-m10", 5, "Two transactions each waiting on a lock the other holds"},

	// sql-m11 (5 questions)
	{"sql-m11", 1, "A function must return a value"},
	{"sql-m11", 2, "PL/pgSQL"},
	{"sql-m11", 3, "Yes"},
	{"sql-m11", 4, "Returning a value from a procedure"},
	{"sql-m11", 5, "Logic runs close to the data, reducing round trips"},

	// sql-m12 (5 questions)
	{"sql-m12", 1, "A junction/join table"},
	{"sql-m12", 2, "A foreign key on the \"many\" side"},
	{"sql-m12", 3, "A foreign key with a UNIQUE constraint"},
	{"sql-m12", 4, "Deletes child rows when the parent is deleted"},
	{"sql-m12", 5, "Entities, attributes and their relationships"},

	// sql-m13 (5 questions)
	{"sql-m13", 1, "To reduce data redundancy"},
	{"sql-m13", 2, "Atomic values, no repeating groups"},
	{"sql-m13", 3, "Partial dependencies on part of a composite key"},
	{"sql-m13", 4, "Transitive dependencies"},
	{"sql-m13", 5, "To reduce joins and speed up reads"},

	// sql-m14 (5 questions)
	{"sql-m14", 1, "WITH"},
	{"sql-m14", 2, "JSONB"},
	{"sql-m14", 3, "It is stored in binary form and can be indexed"},
	{"sql-m14", 4, "RECURSIVE"},
	{"sql-m14", 5, "ROW_NUMBER()"},

	// sql-m15 (5 questions)
	{"sql-m15", 1, "Object-Relational Mapper"},
	{"sql-m15", 2, "SQL injection"},
	{"sql-m15", 3, "Use parameterised queries"},
	{"sql-m15", 4, "Version-controlled, repeatable schema changes"},
	{"sql-m15", 5, "Opening connections is expensive, so they are reused"},

	// sql-m16 (5 questions)
	{"sql-m16", 1, "A separate order_items table referencing orders"},
	{"sql-m16", 2, "Product prices change; the order must record the price paid"},
	{"sql-m16", 3, "Many-to-many, via an enrolments table"},
	{"sql-m16", 4, "A system-generated identifier with no business meaning"},
	{"sql-m16", 5, "Foreign key columns used in joins"},

	// sql-m17 (5 questions)
	{"sql-m17", 1, "Use a subquery with MAX and a WHERE excluding the top value, or DENSE_RANK"},
	{"sql-m17", 2, "RANK leaves gaps after ties; DENSE_RANK does not"},
